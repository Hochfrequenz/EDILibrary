...
namespace EDILibrary
{
    public class EdiJsonMapper
    {
        ...
        public async Task<string> ParseToJson(
            ...
            string includeEmptyValues = null
        )
        {
            return (
                await ParseToJsonWithVersion(
                    edi,
                    packageVersion.ToEdifactFormatVersion(),
                    includeEmptyValues
                )
            ).EDI;
        }

        public async Task<JsonResult> ParseToJsonWithTemplates(
            string edi,
            EdifactFormatVersion? packageVersion,
            string ediTemplate,
            string ediTreeTemplate,
            string ediJsonTemplate,
            string includeEmptyValues = null
        )
        {
            var ediInfo = EDIHelper.GetEdiFileInfo(
            ...
            {
                package = new Tuple<EdifactFormat?, string>(
                    null,
                    packageVersion.Value.ToLegacyVersionString()
                );
            }
            else
            {
            ...
            ParseObject(
                jsonResult,
                resultObject as IDictionary<string, object>,
                mappings,
        ...
        public async Task<JsonResult> ParseToJsonWithVersion(
            string edi,
            EdifactFormatVersion? packageVersion,
            string includeEmptyValues = null
        )
        {
            ...
            try
            {
                mappingJson = await _loader.LoadJSONTemplate(
                    package.Item1,
                    package.Item2,
                    ediInfo.Format + ".json"
                );
            ...
            return await ParseToJsonWithTemplates(
                edi,
                packageVersion,
                templateString,
                treeString,
        ...
        public async Task<string> CreateFromJson(
            string jsonInput,
            string pid,
            string formatPackage,
            TimeZoneInfo localTime,
            MAUS.Anwendungshandbuch? ahb,
        ...
        )
        {
            return await CreateFromJson(
                jsonInput,
                pid,
                formatPackage.ToEdifactFormatVersion(),
                localTime,
                ahb,
                convertFromUTC
            );
        ...
        public string CreateFromJsonWithTemplates(
            string jsonInput,
            string pid,
            EdifactFormatVersion formatPackage,
            string ediJsonTemplate,
        ...
        )
        {
            ...
            return MappingHelper.ExecuteMappings(
                result,
        ...
        )
        {
            ...
            try
            {
                jsonBody = await _loader.LoadJSONTemplate(
                    format,
            ...
            if (json != null)
            {
                ...
                foreach (var step in json.Property("steps").Value)
                {
                    foreach (
                        var prop in (
                            (step as JObject)?.Property("fields").Value as JObject
                        ).Properties()
                    )
                    {
        ...
        public async Task<string> CreateFromEdiJson(
            string jsonInput,
            string pid,
            EdifactFormatVersion formatPackage,
            TimeZoneInfo localTime,
        ...
        protected static void ParseObject(
            JObject value,
        ...
        )
        {
            if (value is null)
            {
                ...
                throw new ArgumentNullException(
                    nameof(value),
                    "This might be due to a missing tree object. Are you sure that template and tree have been loaded correctly?"
                );
            }
            foreach (var prop in value.Properties())
            {
                var deps = mappings
                    .Where(map => FindDependentObject(map, prop.Name, out var propVal) != null)
                    .ToList();
                if (deps.Any())
                {
        ...
        protected static bool FindMask(
            JArray mask,
            string maskKey,
            MAUS.SegmentGroup? ahb,
            Stack<string> parentPath
        )
        {
            if (mask == null)
            {
            ...
            foreach (var maskEntry in mask)
            {
                if (
                    CompareKey(
                        (maskEntry as JObject).Property("key")?.Value.Value<string>(),
                        maskKey
                    )
                )
                {
        ...
        protected static dynamic CreateMsgJSON(
            JObject input,
            JArray mapping,
            JArray mask,
            MAUS.SegmentGroup? ahb,
            List<MAUS.Segment>? segments,
            bool virtualChild,
            Stack<string> parentPath,
            out bool createInParent,
            bool convertFromUTC = false
        )
        {
            ...
            foreach (var prop in input.Properties())
            {
                ...
                var deps = mapping
                    .Where(map => FindObjectByKey(map, prop.Name, out var propVal, false) != null)
                    .ToList();
                if (deps.Any())
                {
                    ...
                    if (foundObj is not null)
                    {
                        ...
                        if (ahb is not null || localAhb is not null)
                        {
                            ...
                            )
                            {
                                ...
                                {
                                    localAhb = localAhb.SegmentGroups.First(s =>
                                        s.Discriminator == sg
                                    );
                                }
                                else
                                ...
                                localAhb?.Segments.Any(s =>
                                    s.SectionName.Replace(" ", "").Dehumanize()
                                    == key.Replace(" ", "").Dehumanize()
                                ) == true
                            ) // check segments
                            {
                                bool notFoundAnySegment = true;
                                foreach (
                                    var segment in localAhb.Segments.Where(s =>
                                        s.SectionName.Replace(" ", "").Dehumanize()
                                        == key.Replace(" ", "").Dehumanize()
                                    )
                                )
                                {
                                    ...
                                    {
                                        if (
                                            segment.DataElements.Any(s =>
                                                s.DataElementId.Dehumanize() == id.Dehumanize()
                                            )
                                        )
                                        {
                                            notFoundAnySegment = false;
                    ...
                    {
                        var newMappings = (
                            (deps.First() as JObject).Property("requires").Value as JArray
                        )
                            .Select(req =>
                            {
                                if (
                                    (req as JObject).Properties().FirstOrDefault()?.Value as JArray
                                    != null
                                )
                                {
                                    return (req as JObject).Properties().FirstOrDefault()?.Value
                                        as JArray;
                                }
                        ...
                        newArray = newMappings
                            .Where(map => map != null)
                            .Aggregate(newArray, (current, map) => new JArray(current.Union(map)));
                        ...
                        if (!newArray.Any())
                        {
                            ...
                            if (subObj.SelectToken("_meta") != null)
                            {
                                ...
                                (returnObject as IDictionary<string, object>).Add(
                                    ...
                                    new ScriptHelper { useLocalTime = convertFromUTC }.FormatDate(
                        ...
                        else
                        {
                            ...
                            if (
                                FindMask(mask, prop.Name, ahb, parentPath)
                                || prop.Name == "Dokument"
                                || prop.Name == "Nachricht"
                                || virtualGroup
                            )
                                ...
                                if (prop.Value.Type == JTokenType.Array)
                                {
                                    ...
                                    foreach (var sub in prop.Value as JArray)
                                    {
                                        if (sub is JObject)
                                        {
                                            ...
                                            List<MAUS.Segment?> localSegment =
                                                new List<MAUS.Segment?>();
                                            ...
                                            if (localAhb is not null)
                                            {
                                                ...
                                                {
                                                    foreach (
                                                        var segment in localAhb?.Segments?.Where(
                                                            s =>
                                            ...
                                            if (
                                                localAhb is not null
                                                && localAhb?.Discriminator != "root"
                                                && !localSegment.Any()
                                            )
                                            {
                                                continue;
                                            ...
                                            {
                                                (
                                                    (returnObject as IDictionary<string, object>)[
                                                        newPropName
                                                    ] as List<dynamic>
                                                ).Add(newSub);
                                            }
                                            else
                                            {
                                                ...
                                                {
                                                    (newObj as IDictionary<string, object>).Add(
                                                        key,
                                                        value
                                                    );
                                                }
                    ...
                    else
                    {
                        if (foundObj.Property("key") != null)
                        {
                            ...
                            if (
                                !FindMask(
                                    mask,
                                    foundObj.Property("key").Value.Value<string>(),
                                    ahb,
                                    parentPath
                                )
                            )
        ...
        protected static JObject ApplyFix(JObject input, JObject fix)
        {
            ...
            foreach (JObject subFix in fix.SelectToken("fix") as JArray)
            {
                ...
                {
                    value = input
                        .SelectToken(subFix.SelectToken("_path")?.Value<string>())
                        ?.Value<string>();
                }
                if (name.Contains("[]."))
                {
        ...
        protected static JObject FindObjectByKey(
            JToken val,
            string key,
            out JToken propVal,
            bool ignoreParentKey
        )
        {
